...
watch(uploadImages, (val) => {
  //扩展每个skus里面的imgs选项
  let imgArr = Array.from(new Set(spu.images.concat(val)));

  //{imgUrl:"",defaultImg:0} 由于concat每次迭代上次，有很多重复。所以我们必须得到上次+这次的总长

  spu.skus.forEach((item, index) => {
    let len = imgArr.length - spu.skus[index].images.length; //还差这么多
    if (len > 0) {
      let imgs = new Array(len);
      imgs = imgs.fill({ imgUrl: "", defaultImg: 0 });
      spu.skus[index].images = item.images.concat(imgs);
    }
  });

  spu.images = imgArr; //去重
  console.log("spu.skus", spu.skus);
})
...
const handlePriceChange = (scope, mpidx, e) => {
  spu.skus[scope.$index].memberPrice[mpidx].price = e;
}

const getMemberLevels = () => {
  proxy.$http({
    url: proxy.$http.adornUrl("/member/memberlevel/list"),
    method: "get",
    ...
    .then(({ data }) => {
      dataResp.memberLevels = data.page.list;
    })
    .catch(e => {
      console.log(e);
    });
...
const showInput = (idx) => {
  ...
  // proxy.$refs['saveTagInput'+idx].$refs.input.focus();
}

const checkDefaultImg = (row, index, img) => {
...
const handleInputConfirm = (idx) => {
  let inputValueVal = inputValue.value[idx].val;
  if (inputValueVal) {
    // dynamicTags.push(inputValue);
    if (dataResp.saleAttrs[idx].valueSelect == "") {
      dataResp.saleAttrs[idx].valueSelect = inputValueVal;
    } else {
      dataResp.saleAttrs[idx].valueSelect += ";" + inputValueVal;
    }
  }
...
const collectSpuBaseInfo = () => {
  //spuBaseForm
  proxy.$refs.spuBaseForm.validate(valid => {
    if (valid) {
      step.value = 1;
      showBaseAttrs();
    } else {
...
const generateSkus = () => {
  ...
  let descartes = descartes_func(selectValues);
  ...
  if (!dataResp.steped[1]) {
    proxy.$http({
      url: proxy.$http.adornUrl(
        `/product/attr/sale/list/${spu.catalogId}`
      ),
      method: "get",
      params: proxy.$http.adornParams({
        page: 1,
      ...
      dataResp.saleAttrs = data.page.list;
...
const showBaseAttrs = () => {
  if (!dataResp.steped[0]) {
    proxy.$http({
      url: proxy.$http.adornUrl(
        `/product/attrgroup/${spu.catalogId}/withattr`
      ),
      method: "get",
      params: proxy.$http.adornParams({})
    }).then(({ data }) => {
...
const submitSkus = () => {
  ...
  proxy.$confirm("将要提交商品数据，需要一小段时间，是否继续?", "提示", {
    ...
    .then(() => {
      proxy.$http({
        url: proxy.$http.adornUrl("/product/spuinfo/save"),
        method: "post",
        data: proxy.$http.adornData(spu, false)
      }).then(({ data }) => {
        if (data.code == 0) {
          proxy.$message({
            type: "success",
